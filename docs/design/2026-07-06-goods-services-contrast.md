...
|---|---|---|---|---|---|---|---|---|---|
| corr | −0.167 | −0.102 | −0.174 | −0.253 | −0.232 | +0.064 | +0.136 | +0.389 | +0.255 |

（2020 年は panel に存在しない。2026-07-06 に本設計者が実データで算出した3桁精度値。）
...
GOLDEN_S0_CORR = {2015: -0.167, 2016: -0.102, 2017: -0.174, 2018: -0.253, 2019: -0.232,
...
値の抽出規則（共通）: `goods_services_contrast.csv` から `spec, bucket, metric=='corr', year_or_window ∈ {'2021','2022','2023','2024'}`（ショック期）または `{'2015'..'2019'}`（プラセボ期）で絞った `value` 列の min と max を取り、小数2桁・符号付き（例 `+0.06〜+0.39`）で表記する。
...
> - **交絡の定量化（財/サービス分解・DEC-022）**: 年次相関コントラストを財のみ／サービスのみに分解すると、ショック期の within-goods 相関は [S1 goods shock min〜max]、within-services は [S1 services shock min〜max]（全39 は +0.06〜+0.39）。[(a) の場合:「全体コントラストの相当部分は財/サービス構成差で説明され、cost-push 固有シグナルとしての解釈はさらに限定される」／(b) の場合:「財内部にも IC 勾配が残り、構成差だけでは説明されない」]。エネルギー公共料金の扱いと 2023-24 補助金の感度は S2/S3（`goods_services_contrast.csv`）。
