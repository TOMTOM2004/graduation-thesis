...
#   小売価格が BOJ CGPI 集計輸入物価から切り離されている財。
#   Shift-Share 回帰の前提（輸入コスト増 → 価格転嫁）が成立しない外れ値。
#
#   対象: 衣料（0083、IC=0.876）・履物類（0098、IC=0.911）
#   根拠（財務省貿易統計 e-Stat 概況品別国別表、2016-2024年データより）:
#     - 中国単体シェア: 衣類 46–64%、履物 41–55%（期間内変動あり、低下傾向）
#     - 競争的アジアサプライヤー合計（中国・ベトナム・バングラデシュ・
#       カンボジア・ミャンマー）: 衣類 81–84% で安定 → 2016-2024 を通じ不変
#     - これらサプライヤーの価格は生産コスト・為替競争力で決まり、
#       BOJ CGPI（エネルギー・金属・化学主導）と独立して動く（価格デカップリング）
#     - よって IC は高くても「輸入コスト上昇 → 小売価格転嫁」パスが機能しない
#   データソース: data/processed/trade-stats/china_import_share_apparel.csv
#   識別戦略上の位置づけ（DEC-010 参照）:
#     モデルの前提が成立しない財を事前定義として除外 → post-hoc 操作ではない
