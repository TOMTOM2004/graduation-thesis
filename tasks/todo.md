...
- [ ] **② Phase 2a を group-specific shift-share に強化（A-4）**: 集計 P_import × IC を**5グループ別 IC_{c,g}×P_{g,t}** に分解 → 識別変動増・グループ別転嫁の異質性（貢献増）・**Phase 3 較正の的（cost-push のグループ別定義）になる**。手順③Phase3再較正の前提。`cost_push_panel.py` 拡張
- [ ] **③ Phase 3 を cost-push 成分に再較正（C-4 根本変更）**: ×β スケーリングを廃し、Phase 2a(group別)が定義する cost-push CPI に対し転嫁率＋動学を**財/グループ別に較正**（日本2021-24 への calibration 許容）。総合CPIは追わない（需要モジュールは decouple・将来拡張）。Leontief 構造は維持。下の「Phase3 Koyck再生成」はこの一部に統合
- [ ] **B-4 輸入含有率 IC>1 の是正**: `sector_import_content.csv` で sector 61=1.07 等（含有率>1 は不可能）。`io_utils` の (I−A)⁻¹ 適用を競争輸入型 (I−(I−M̂)A)⁻¹ 等に再構成 or 検証。Phase 2a の IC・Phase 3 の content に波及（A-4 の前に or 同時に）
